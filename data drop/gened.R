...
p_gened <-results_gened %>%
  select(condition, gen_ed_label) %>%
  group_by(condition) %>%
  count(gen_ed_label) %>%
  mutate(cumulative = cumsum(n),
         proportion = n/sum(n)) %>%
  filter(!is.na(gen_ed_label))
  
ggplot(p_gened, aes(x = proportion, y = gen_ed_label, fill = gen_ed_label)) +
  geom_density_ridges() +
  scale_x_continuous(labels = scales::percent) +
  theme_ridges() + 
  theme(legend.position = "none") +
  labs(x = "Response %", y = NULL,
       title = "How strong is your district's general education supports for students with disabilities?")
